...
public class View extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		for(int i = 0; i < model.getBoard().getNumRows()-1; i++) {
			// Para cada um dos caracteres da linha...
			for(int k = 0; k < model.getBoard().getNumCols()-1; k++) {
				
				if(model.getBoard().isWall(i,k)) {
					g.setColor(Color.BLACK);
					g.fillRect(CELL_SIZE*k, CELL_SIZE*i, CELL_SIZE, CELL_SIZE);
